...
- Do not touch any other issue. Skip formatters and project-wide lint runs except the gate.
...
- Branch A: branch `agent/issue-<N>` from fresh origin/main; TDD per AGENTS.md;
  ...
  then report blocked); merge; then `git fetch origin && git fetch origin main:main`.
...
- Final message, exactly:
  ISSUE <N>: <merged (PR #x) | triaged (label) | closed (wontfix) | blocked>
  Notes: <1–3 lines>
```
...
1. Verify the claim (merged? labels? comment posted?) with one `gh` call — agents
   report honestly but the orchestrator owns the queue.
2. Mark the issue done; if it unblocked dependents, reorder the queue.
3. Spawn the next subagent. Repeat until the queue is strictly serial, one agent at
   a time.
